...
class ProjectRetriever:
    ...
    def _backfill_missing_embeddings(self, chunks: list[dict]) -> None:
        missing = [chunk for chunk in chunks if not chunk.get("embedding") and (chunk.get("content") or "").strip()]
        if not missing:
            return

        embeddings = embed_texts([chunk["content"] for chunk in missing], kind="passage")
