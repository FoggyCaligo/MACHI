...
class UploadedProfileEvidenceStore:
    ...
    def _resolve_topic(self, topic: str | None, topic_id: str | None) -> tuple[str | None, str | None]:
        ...
        if not resolved_topic_id and normalized_topic and normalized_topic.lower() != "general":
            resolved_topic_id = self.topic_store.ensure_topic(
                normalized_topic,
                source="UploadedProfileEvidenceStore",
                confidence=0.65,
            )
    ...
    def add(
        ...
        with get_conn() as conn:
            conn.execute(
                """
                INSERT INTO uploaded_profile_evidence (
                    id,
                    source_id,
                    source_file_path,
                    evidence_type,
    ...
    def delete_by_source(self, source_id: str) -> None:
        with get_conn() as conn:
            conn.execute("DELETE FROM uploaded_profile_evidence WHERE source_id = ?", (source_id,))
            conn.commit()
    ...
    def list_by_source(self, source_id: str) -> list[dict]:
        with get_conn() as conn:
            rows = conn.execute(
                """
                SELECT *
                FROM uploaded_profile_evidence
                WHERE source_id = ?
    ...
    def list_unapplied_by_source(self, source_id: str) -> list[dict]:
        with get_conn() as conn:
            rows = conn.execute(
                """
                SELECT *
    ...
    def mark_applied(
        ...
        with get_conn() as conn:
            conn.execute(
                """
                UPDATE uploaded_profile_evidence
