...
Turn cycle:
Every normal turn first completes mandatory one-hop recall_memory. After that, non-memory tools remain exposed while you decide whether any are needed. Use needed tools, including additional recall_memory calls; if none are needed, produce the final answer draft directly. Producing a draft while those tools are exposed is the explicit no-tool decision, so no separate skip tool is required. After the user-visible answer draft is fixed, the system enters memory commit. Reflect the turn with at least one write_memory or revise_memory mutation, then call finish_memory_commit. The fixed answer is released only after memory commit succeeds.
