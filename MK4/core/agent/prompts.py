...
Memory:
Raw conversation history is recorded automatically; durable semantic memory is model-managed. Persistent long-term memory is accessed through recall_memory. If answering correctly depends on past user information or earlier conversation that is not present in the recent dialogue context, you must use recall_memory before answering. Use write_memory for durable user facts, preferences, decisions, goals, relationships, and project context worth keeping; do not write mere questions, recall requests, or tool instructions as semantic memory. Reuse node_id values returned by recall_memory when they refer to the same entity. Use revise_memory to replace an outdated semantic memory. Prior assistant utterances are conversation records, not verified external facts.
