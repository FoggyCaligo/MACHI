...
`MK4`는 그래프를 사고의 본체로 두던 흐름에서 한 걸음 물러나, **그래프를 기억 전용 인프라로 단순화하고 LLM이 도구를 오케스트레이션하는 실행형 에이전트**다.

이전 구조는 그래프 위에서 직접 사고하는 데 강점이 있었지만, 그만큼 파이프라인이 무거워지고, 응답 경로가 복잡해지며, 유지보수 부담도 커진다. `MK4`는 이 문제를 해결하기 위해 그래프의 역할을 다시 줄인다.
...
- UI에서는 파일 첨부를 지원하며, 업로드된 파일은 `.mk4_uploads/` 아래에 저장된다.
...
즉 `MK4`에서는 그래프가 먼저 생각하고 LLM이 번역하는 것이 아니라, **LLM이 계획하고 그래프는 기억 저장소로 지원**한다.

## 기억구조

`MK4`의 기억 구조는 이전 세대보다 훨씬 단순하다.
...
즉 `MK4`의 기억 구조는 **사용자 anchor + 장기 그래프 + 도구 기반 회수 + 턴 단위 기억 활성도 상태**로 요약할 수 있다.

### 그래프를 장기 기억으로 유지하는 이유

`MK4`가 기억 저장소로 그래프를 유지하는 이유는 단순한 문자열 검색을 넘어서, 반복되는 개념을 공유하고 그 개념이 형성한 주변 관계를 명시적으로 보존하기 위해서다. 문장 단위 VectorDB와 비교했을 때 다음 두 가지 장점을 기대할 수 있다.
...
현재 최근 대화 기본 개수는 `MK4_RECENT_MESSAGE_LIMIT=10`개 메시지로, 사용자와 assistant 메시지를 한 쌍으로 보면 약 5턴이다. 직전 도구 작업은 도구명, 축약 인자, 성공 여부, 오류와 짧은 결과 요약을 중복 제거한 뒤 최대 5개 보관한다. 현재 턴에서 실행된 도구의 결과도 `tool_history`에 도구별로 압축되어 다음 모델 라운드에 전달된다. 모델에는 도구 이름만 기본 제공하고, 전체 설명과 schema는 필요한 경우 `tool_manual`로 조회한다.

그래프 기억 요약은 기본 최대 5개(`MK4_MEMORY_SUMMARY_LIMIT=5`)만 자동 주입한다. 현재 발화와의 직접 관련성 또는 그래프 활성도 신호가 `MK4_MEMORY_SUMMARY_MIN_SIGNAL` 기준을 통과한 항목만 포함하므로, 관련 기억이 부족하면 5개를 억지로 채우지 않는다. 각 기억은 원문 노드나 edge 목록 전체가 아니라 **중심 기억 + 핵심 관계 최대 4개 + 회수 근거·출처**의 작은 서브그래프로 압축된다. 자동 요약만으로 과거 발언·선호·결정·프로젝트 문맥이 부족하면 모델은 기억이 없다고 답하기 전에 `graph_search`로 추가 조회한다.

텍스트 파일에서 만들어지는 노드는 기본적으로 상위 70%, 최대 24개만 유지한다. 이 값은 `MK4_FILE_TEXT_NODE_KEEP_RATIO`, `MK4_FILE_TEXT_NODE_MAX_ITEMS`로 조정할 수 있다. 후처리 입력은 기본 8,000자로 제한되며 `MK4_FILE_TEXT_ACTIVATION_MAX_CHARS`로 조정한다. 파일 문맥 노드는 출처 추적을 위해 그래프 DB에는 기록되지만 `suppress_from_summary`가 설정되므로 사용자 발화·사실 기반 장기 기억 summary 후보로 자동 주입되지 않는다.
...
JSON 파싱 실패가 발생하면 서버 콘솔의 `[MK4 model] output_parse_failed` 로그에 구체적인 파싱 오류, 원문 길이, 실패 출력 preview가 기록된다. preview 기본 상한은 2,000자이며 `MK4_MODEL_FAILURE_PREVIEW_CHARS=0`으로 원문 기록을 끌 수 있다. Ollama의 HTTP 400 요청 거부는 JSON 파싱 실패와 별도로 표시된다.
...
웹 UI와 API는 `MK4/.env`의 `MK4_ALLOWED_LOGIN_IDS` 목록에 등록된 문자열만 허용한다. 값은 JSON 배열 또는 쉼표 구분 문자열로 추가할 수 있다. `default-user`와 목록에 없는 값은 거부되고 UI에는 로그인 모달이 표시된다. 로그인에 성공하면 서버가 `HttpOnly`, `SameSite=Strict` 세션 쿠키를 발급하며, `/chat`의 그래프 사용자 ID는 요청 본문이 아니라 로그인 정보에서 강제한다.
...
`MK4/.env`는 저장소에 커밋되지 않는다. 예시는 다음과 같다.

```dotenv
MK4_ALLOWED_LOGIN_IDS=["owner-secret-id","family-id","friend-id"]
MK4_OWNER_LOGIN_ID=owner-secret-id
MK4_OWNER_GRAPH_USER_ID=existing-graph-user-id
MK4_MAX_ACTIVE_SESSIONS=3
```

`MK4_OWNER_LOGIN_ID`와 일치하는 한 계정만 `owner` 권한을 얻는다. 다른 허용 ID는 로그인 문자열의 SHA-256에서 안정적인 그래프 사용자 키를 파생하므로 서로 기억이 섞이지 않는다. 허용 ID 문자열을 바꾸면 새 그래프 사용자로 취급된다. 활성 로그인 세션은 `MK4_MAX_ACTIVE_SESSIONS`개로 제한되며 기본값은 3이다. 같은 브라우저에서 다시 로그인하면 기존 세션을 교체하므로 슬롯을 추가 소비하지 않는다.

브라우저에는 ID 원문 대신 무작위 `HttpOnly` 세션 토큰만 저장한다. 서버는 토큰의 SHA-256 해시를 `MK4/data/sessions.db`에 보관하므로 브라우저나 MK4를 다시 시작해도 기본 168시간 동안 로그인이 유지된다. `.env` 허용 목록에서 ID를 제거한 뒤 서버를 다시 시작하면 그 계정의 기존 세션도 다음 요청에서 자동 폐기된다. `MK4_SESSION_TTL_HOURS`로 유지 시간을 조정할 수 있다.
...
이 작업은 해당 그래프 사용자 anchor, 사용자 소유 노드, 사용자 ID가 기록된 edge와 그 결과 고아가 된 concept 노드를 삭제한다. 복구가 필요하면 실행 전에 `MK4/data/memory.db`를 백업해야 한다.

서버는 기본적으로 안전한 로컬 주소 `127.0.0.1:8010`에 바인딩한다. LAN에서 직접 열어야 할 때만 `MK4_SERVER_HOST=0.0.0.0`을 명시한다. 인터넷 HTTPS 주소 뒤에서 사용할 때는 `MK4_SESSION_COOKIE_SECURE=true`로 실행해야 한다.
...
스크립트는 MK4를 `127.0.0.1:8010`에서 숨김 프로세스로 실행하고 secure session cookie를 활성화한 뒤 `tailscale funnel 8010`을 연결한다. Funnel은 이 장치의 예측 가능한 `https://<장치>.<tailnet>.ts.net` 주소를 공개 인터넷에 제공한다. 장치가 켜져 있고 Tailscale과 MK4가 실행 중일 때만 접속할 수 있으며, Funnel 자체가 공개 인터넷 경로라는 점은 변하지 않는다.
...
```powershell
$env:MK4_OLLAMA_MODEL_NAME="gemma4:e4b"
$env:MK4_OLLAMA_IMAGE_MODEL_NAME="gemma4:12b"
$env:MK4_OLLAMA_IMAGE_FALLBACK_MODEL_NAME="gemma4:12b"
```
...
`MK4`는 `playlist2` 같은 다른 레포에서 그래프 메모리와 도구 오케스트레이션 엔진으로 가져다 쓸 수 있다.

이때 호스트 레포별로 아래 경로를 분리해서 지정하는 것이 좋다.

- `MK4_WORKSPACE_ROOT`
- `MK4_DB_PATH`
- `MK4_SENTENCE_BREAKER_DB_PATH`

자세한 예시는 `docs/using_mk4_from_other_repo.md`를 참고하면 된다.
