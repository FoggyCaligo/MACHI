...
def test_system_prompt_makes_frozen_required_tools_explicit() -> None:
    ...
    assert "Every listed tool must succeed before you release a final answer" in SYSTEM_PROMPT
    assert "automatic_memory_context is framework context and never counts as execution" in SYSTEM_PROMPT
    assert "Tools not listed remain available" in SYSTEM_PROMPT
...
def test_system_prompt_memory_matches_pre_memory_freeze() -> None:
    ...
    assert "If recall_memory is listed in frozen_tool_requirements.required_tools, execute recall_memory" in SYSTEM_PROMPT
