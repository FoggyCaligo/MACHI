...
class SummaryStore:
    def upsert_topic_summary(self, topic: str, content: str, source_episode_ids: list[str]):
        ...
        with connection_context() as conn:
            ...
            if existing:
                conn.execute(
                    "UPDATE summaries SET content = ?, source_episode_ids = ?, updated_at = ? WHERE id = ?",
                    (content, encoded, now, existing['id']),
                )
                return existing['id']
            summary_id = str(uuid.uuid4())
            conn.execute(
                "INSERT INTO summaries (id, topic, content, source_episode_ids, updated_at) VALUES (?, ?, ?, ?, ?)",
                (summary_id, topic, content, encoded, now),
            )
            return summary_id
