...
class ProjectProfileEvidenceService:
    ...
    def _select_documents(self, project_id: str) -> list[dict]:
        ...
        return self.passage_selector.filter_profile_documents(files, max_docs=8)
    ...
    def _build_extract_user_prompt(
        ...
        *,
        ...
        selected_passages, selection_meta = self.passage_selector.select_profile_passages_across_documents(
            documents=documents,
        ...
        selection_header = (
            f"[selection]\ncount={selection_meta.get('selected_passage_count', 0)} "
            f"chars={selection_meta.get('selected_chars', 0)} "
            f"docs={selection_meta.get('document_count', len(documents))} "
            f"mode={selection_meta.get('selection_mode', 'profile_passages_across_documents')}"
        )
