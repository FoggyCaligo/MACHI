...
class ProfileMemorySyncService:
    ...
    def _memory_value_signals(self, evidence: dict) -> list[str]:
        ...
        signals: list[str] = []
        if str(evidence.get("source_strength") or "") == "explicit_self_statement":
            signals.append("explicit_self_statement")
        if self._has_first_person_assertion(combined):
            signals.append("first_person_assertion")
        if self._has_strong_confidence(combined):
            signals.append("strong_confidence")
        if self._has_concrete_background(combined):
            signals.append("concrete_background")
        return signals

    def _is_direct_confirmable_evidence(self, evidence: dict) -> bool:
        confidence = float(evidence.get("confidence") or 0.0)
        if confidence < TOPIC_CONFIRM_MIN_CONFIDENCE:
            return False
        return bool(self._memory_value_signals(evidence))
    ...
    def _build_candidate_clusters(self, evidences: list[dict]) -> list[dict]:
        ...
        for evidence in evidences:
            ...
            if self._is_direct_confirmable_evidence(evidence):
                cluster["direct_confirm_count"] += 1
            cluster["memory_value_hits"] += len(self._memory_value_signals(evidence))
