...
IDEA1 원설계의 "노드 = 체감된 개념" 철학을 구현하는 핵심 컴포넌트다.

주의:
- ConceptDifferentiation은 개념 정리의 한 축이다.
- 실제 사고 루프에는 대칭 축인 ConceptMerge(중복/동의/표현 차이 통합)가 함께 필요하다.
- 분화만 있고 병합이 없으면 그래프가 파편화되고, 병합만 있고 분화가 없으면 과도하게 뭉친다.
