...
class ChatJobStore:
    """Request-detached chat jobs owned by graph user identity."""
    ...
    def create(self, *, graph_user_id: str) -> ChatJob:
        self._cleanup()
        now = time.time()
        job = ChatJob(
    ...
    def snapshot_for(self, *, job_id: str, graph_user_id: str) -> dict[str, Any] | None:
        self._cleanup()
        job = self._jobs.get(job_id)
        if job is None or job.graph_user_id != graph_user_id:
            return None
        return {
            "ok": True,
            "job_id": job.job_id,
    ...
    def lock_for(self, graph_user_id: str) -> asyncio.Lock:
        lock = self._user_locks.get(graph_user_id)
        if lock is None:
            lock = asyncio.Lock()
            self._user_locks[graph_user_id] = lock
        return lock
