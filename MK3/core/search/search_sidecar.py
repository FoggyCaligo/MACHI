...
class WikipediaSearchBackend:
    ...
    def _search_wikipedia(
        ...
        *,
        ...
        request = Request(base, headers={'User-Agent': 'MK3-SearchSidecar/0.1'})
        try:
    ...
    def _fetch_summary_passages(self, *, title: str, lang: str, timeout_seconds: float) -> list[str]:
        ...
        request = Request(summary_url, headers={'User-Agent': 'MK3-SearchSidecar/0.1'})
        try:
            with urlopen(request, timeout=timeout_seconds) as response:
                payload = json.loads(response.read().decode('utf-8'))
        except Exception:
...
class DuckDuckGoSearchBackend:
    ...
    def search(self, query: str, *, max_results: int, timeout_seconds: float) -> SearchBackendResult:
        request = Request(
            ...
            headers={
                'User-Agent': 'Mozilla/5.0 (compatible; MK3-TrustedSearch/0.1)',
...
class SearchSidecar:
    ...
    def _fetch_passages_from_url(self, url: str, *, query: str, title: str) -> list[str]:
        request = Request(
            url,
            headers={
                'User-Agent': 'Mozilla/5.0 (compatible; MK3-SearchSidecar/0.1)',
                'Accept-Language': 'ko,en;q=0.8',
            },
        )
