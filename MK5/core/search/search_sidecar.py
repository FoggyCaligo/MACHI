...
class SearchBackend(Protocol):
    def search(self, query: str, *, max_results: int, timeout_seconds: float) -> SearchBackendResult | list[SearchEvidence]: ...
...
class WikipediaSearchBackend:
    ...
    def _search_wikipedia(self, query: str, *, lang: str, max_results: int, timeout_seconds: float) -> tuple[list[SearchEvidence], str | None]:
        ...
        for index, title in enumerate(titles[:max_results]):
            ...
            evidences.append(SearchEvidence(title=str(title), snippet=str(snippet or ''), url=str(url or ''), provider=f'wikipedia-{lang}', domain='wikipedia.org', trust_hint='reference', source_provenance=f'wikipedia:{lang}', passages=passages, metadata={'query': query, 'lang': lang}))
...
class DuckDuckGoSearchBackend:
    ...
    def search(self, query: str, *, max_results: int, timeout_seconds: float) -> SearchBackendResult:
        request = Request(f'{self.search_url}?q={quote(query)}', headers={'User-Agent': 'Mozilla/5.0 (compatible; MK5-TrustedSearch/0.1)', 'Accept-Language': 'ko,en;q=0.8'})
        try:
            with urlopen(request, timeout=timeout_seconds) as response:
    ...
    def _parse_results(self, payload: str, *, max_results: int) -> list[SearchEvidence]:
        ...
        for index, match in enumerate(anchor_pattern.finditer(payload)):
            ...
            evidences.append(SearchEvidence(title=title, snippet=snippet, url=resolved_url, provider='duckduckgo-web', domain=domain, trust_hint='web_search', source_provenance=f'web:duckduckgo:{domain or "unknown"}', metadata={'domain': domain}))
...
class SearchSidecar:
    ...
    def run(self, *, message: str, thought_view: ThoughtView, conclusion: CoreConclusion, model_name: str) -> SearchRunResult:
        ...
        if not decision.need_search:
            return SearchRunResult(attempted=False, decision=decision, planning_attempted=False)
        try:
            plan = self.query_planner.plan(message=message, thought_view=thought_view, conclusion=conclusion, decision=decision)
        ...
        return SearchRunResult(attempted=bool(results), decision=decision, plan=plan, results=results, planning_attempted=True, provider_errors=provider_errors)
    ...
    def _ensure_evidence_passages(self, item: SearchEvidence, *, query: str) -> None:
        if item.passages:
            item.passages = item.passages[: self.evidence_passage_limit]
            return
        if item.provider.startswith('wikipedia-'):
            if item.snippet:
                item.passages = [item.snippet]
            return
        if not item.url:
            if item.snippet:
                item.passages = [item.snippet]
            return
        try:
            request = Request(item.url, headers={'User-Agent': 'Mozilla/5.0 (compatible; MK5-TrustedSearch/0.1)', 'Accept-Language': 'ko,en;q=0.8'})
            with urlopen(request, timeout=self.evidence_fetch_timeout_seconds) as response:
                content_type = response.headers.get('Content-Type', '')
                if 'text/html' not in content_type:
                    raise ValueError(f'unsupported content-type: {content_type}')
                payload = response.read().decode('utf-8', errors='ignore')
        except Exception as exc:
            fallback = item.snippet or ''
            item.passages = [fallback] if fallback else []
            item.metadata = {**item.metadata, 'passage_fetch_error': str(exc)}
            return
        passages = self._extract_passages_from_html(payload, query=query)
        if not passages and item.snippet:
            passages = [item.snippet]
        item.passages = passages[: self.evidence_passage_limit]

    def _extract_passages_from_html(self, payload: str, *, query: str) -> list[str]:
        text = re.sub(r'<script.*?</script>|<style.*?</style>', ' ', payload, flags=re.IGNORECASE | re.DOTALL)
        text = re.sub(r'<[^>]+>', ' ', text)
        text = html.unescape(text)
        ...
        for sentence in sentences:
            ...
            if score <= 0:
                continue
            ranked.append((score, compact[:320]))
        ranked.sort(key=lambda item: (-item[0], len(item[1])))
        return [text for _, text in ranked[: self.evidence_passage_limit]]

    def _normalize_backend_response(self, response: SearchBackendResult | list[SearchEvidence], *, query: str) -> SearchBackendResult:
        if isinstance(response, SearchBackendResult):
            return response
