...
class SearchNeedEvaluator:
    ...
    def _grounding_scope_nodes(self, thought_view: ThoughtView) -> list[Node]:
        ...
        for node in thought_view.nodes:
            ...
            if self._node_has_grounding(node):
                prioritized.append(node)
            else:
                fallback.append(node)
        ordered = prioritized + fallback
        return ordered[: self.grounding_scope_limit]
    ...
    def _slot_is_covered(self, nodes: list[Node], slot: RequestedSlot, grounded_entities: dict[str, bool]) -> bool:
        ...
        return self._has_rich_entity_evidence(supporting_nodes)
    ...
    def _has_rich_entity_evidence(self, nodes: list[Node]) -> bool:
        if not nodes:
            return False
        long_passage_count = 0
        distinct_sources: set[str] = set()
        for node in nodes:
            text = self._node_text(node)
            if len(text) >= 80:
                long_passage_count += 1
            payload = node.payload if isinstance(node.payload, dict) else {}
            source = ' '.join(str(payload.get('source') or payload.get('title') or '').split()).strip().lower()
            if source:
                distinct_sources.add(source)
        return long_passage_count >= 1 or len(distinct_sources) >= 2
