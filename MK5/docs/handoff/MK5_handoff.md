...
- `grounded_terms`, `missing_terms`, `missing_aspects`는 일부 전달되지만, 실제 응답 단정 수준을 충분히 강하게 통제하지는 못한다.
...
- chat UI search debug가 `requested_slots`, `covered_slots`, `missing_slots`, `issued_slot_queries`를 정상 렌더링하는지
- integration 테스트 fixture가 현재 `SearchEvidence` 계약과 맞는지
- verbalizer가 search gap을 자연스럽게 메우는 방향으로 과장하지 않는지
