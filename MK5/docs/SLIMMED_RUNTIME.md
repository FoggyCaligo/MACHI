...
## Step A refactor update
- Pipeline order is now `ingest -> presearch direct access/search -> search ingest -> activation -> think -> verbalization`.
- The pre-search activation/thinking pass was removed.
- Final activation reuses ingest blocks instead of segmenting the same input twice in the pipeline.
- Assistant full-answer ingest was removed; only runtime/debug metadata remains.
