...
**역할:** 빈 곳을 채우고, 세계그래프를 갱신하며, 정체성과 지식을 회상한다.
...
Update 루프
  ...
  │
  ├─ 정체성 및 기억 활성화 (Identity Activation)
  │    - 사용자 입력 키워드를 ANCHOR_USER 노드에 자동 귀속 (Structural Attribution)
  │    - 참여자 앵커 주변의 국부 그래프를 사고 공간으로 소환 (Identity Recall)
  │
  ├─ ConceptDifferentiation & Merge
  │    - 임시 사고 그래프 내 유사 노드 쌍 탐지 및 증분 비교 최적화
  │    - 분화: 공통 의미 노드 생성 (abstract, 레이블 없음)
  │    - 병합: 유사도 0.94 이상 노드 통합 및 단어 레이어 재매핑
  ...
  │
  └─ WorldGraph 강한 커밋 (새로 추가된 노드/엣지, 병합 결과 반영)
```
...
| ConceptMerge | ✅ 구현 완료 | `core/thinking/concept_merge.py` |
| 정체성 및 기억 활성화 | ✅ 구현 완료 | `core/thinking/thought_engine.py` |
| 주제 연속성 판단 | ✅ 구현 완료 | `app/pipeline.py`, `core/thinking/thought_engine.py` |
| 지식 컨텍스트 확장 | ✅ 구현 완료 | `app/pipeline.py` (`graph_to_lang`) |
