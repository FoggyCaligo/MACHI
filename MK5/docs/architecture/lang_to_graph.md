...
MK5에서 `words`는 단어를 단일 노드로 고정하는 테이블이 아니다.  
...
이 정책은 `MK3`처럼 사용자가 직접 말하지 않은 profile/local context concept이 `사용자 → MK3`, `핵심 키워드: MK3`처럼 현재 발화의 본체로 승격되는 것을 막기 위한 것이다.
...
MK5에서 노드의 의미는 그래프 위치(구조)에 있다. 텍스트 레이블은 부속 정보다.
