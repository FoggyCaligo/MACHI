...
LangToGraph(user_input)
  → TranslatedGraph {
      nodes: [ConceptPointer | EmptySlot, ...]  ← 모든 토큰, 필터링 없음
      edges: [TranslatedEdge, ...]               ← 인접 쌍 neutral, 세계그래프 미반영
    }
  ※ 각 ref에 importance(centroid cosine) 점수 포함
...
Update 루프 (필요 횟수만큼)  ← 세계그래프 갱신 담당
  ...
  ├── 정체성 기반 기억 회상 (Identity Memory Activation) [신규]
  │     - 사용자/AI 앵커 노드 주변의 국부 활성화 그래프(N-hop)를 사고 공간에 자동 로드
  │     - 과거 대화 맥락과 정체성 관련 정보를 구조적으로 소환
  ├── ConceptDifferentiation (유사 노드 분화 + 공통 상위 노드 추출)
  │     - 증분 비교 최적화: 변경된 노드가 포함된 쌍만 검사
  ├── ConceptMerge (동의어/중복 노드 통합) [완료]
  │     - 유사도 0.94 이상인 노드 압축 및 단어 레이어 재매핑
  ├── 세계그래프 커밋 (신규 노드/엣지)
  └── 수렴 판단 → 수렴하면 루프 종료
  │
  ▼
Think 루프 [예정]  ← 목표 지향 추론 담당 (WorldGraph 수정 없음)
  ├── Goal 노드 방향으로 관련 노드 탐색 (spreading activation)
  ├── WorldGraph 이웃 구조를 TempThoughtGraph에 추가 로드
  ├── 목표 방향 활성화 강도 기반 결론 후보 선별
  └── 수렴 판단 → 결론 구조 확정
  │  ※ "불을 끄려면?" → 물, 소화기, 소방수가 결론에 포함되는 단계
  │
  ▼
ConclusionView 구성
  → importance 상위 20% → key_hashes (핵심 키워드)
  → importance 하위 20% → ref_hashes (참고 개념)
  → search_node_hashes (이번 세션 검색 결과 노드, 컨텍스트 격리용)
  │
  ▼
...
Think의 목표는 단순 그래프 업데이트가 아니다.

- 세계그래프를 기준으로 활성 국소그래프 집합을 조작한다.
- Goal 노드에 가장 유사하게 닿는 결론 구조를 찾는다.
- 결론 구조를 GraphToLang으로 언어화한다.

예를 들어 "불을 끄려면?" 입력에서 결론은
"물", "소화기", "소방수"처럼 목표 달성에 기여하는 노드를 포함해야 하며,
"불, 끄다" 수준의 표면 재진술로 끝나면 정책상 불충분하다.

### 5. 분화와 병합의 대칭

개념 형성은 분화만으로 완성되지 않는다.

- ConceptDifferentiation: 뭉친 개념을 분리
- ConceptMerge: 중복/동의/표현 차이 개념을 통합

정책상 두 축은 함께 있어야 의미 그래프가 안정화된다.
...
| ConceptDifferentiation 유사도 | 복합 스코어 (C안) + 적응형 α (이웃 수 기반) + **증분 비교 최적화** |
| 인칭 및 정체성 해결 | **구조적 귀속(Structural Attribution)**: 모든 입력 키워드를 '사용자' 앵커에 자동 연결하여 구조적으로 '나'를 해결 |
| 기억 회상 전략 | **Identity Memory Activation**: 앵커 노드 기반의 국부 활성화 그래프 추출 및 로드 |
| 토큰 중요도 필터링 | **LangToGraph는 필터링 없이 모든 토큰을 반환한다.** 각 ref에 `importance` 점수(centroid cosine)를 in-place 할당. **ThoughtEngine**이 루프 종료 후 importance 기준으로 상위 `TOKEN_IMPORTANCE_NEAR_RATIO=20%` → `key_hashes`(핵심 키워드), 하위 `TOKEN_IMPORTANCE_FAR_RATIO=20%` → `ref_hashes`(참고 개념)로 분류. WorldGraph 저장 임베딩 불사용(그래프 상태가 centroid 오염 방지). 임베딩 없는 경우 레이블 길이 폴백. |
| 지식 컨텍스트 확장 | **GraphToLang 지식 주입**: 신규 검색 결과뿐만 아니라 **핵심 키워드(key_hashes)의 내부 payload**도 함께 로드하여 LLM에게 제공 (할루시네이션 억제) |
| 주제 연속성 판단 | **MK5 포팅**: 키워드 중첩도 기반 4단계 상태(new, continued, related, shifted) 판별 및 시스템 프롬프트 반영 |
