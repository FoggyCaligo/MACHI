...
| 토큰 중요도 필터링 | LangToGraph에서 문장별 **near + far 방식** 적용. centroid 근접 `TOKEN_IMPORTANCE_NEAR_RATIO(15%)` (문장 대표 개념) + centroid 원거리 `TOKEN_IMPORTANCE_FAR_RATIO(15%)` (고유명사·도메인 특이 개념)의 합집합을 노드로 생성. 단순 상위 N% 방식에서 탈락하던 특이 도메인 개념(고유명사 등)을 포착. 임베딩 없는 경우 레이블 길이 폴백. 최소 2개 보장. |
