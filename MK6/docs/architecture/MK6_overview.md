...
| GraphToLang 핵심/참고 분류 | `known_hashes` 폐기. **`key_hashes`(importance 상위 20%) / `ref_hashes`(importance 하위 20%)** 기반으로 교체. ThoughtEngine이 루프 종료 후 `translated.nodes`의 importance 점수로 분류. EmptySlot에서 ingest된 노드도 동일 기준 적용. |
| GraphToLang 검색 컨텍스트 격리 | `ConclusionView.search_node_hashes`: 이번 세션에서 `_ingest_slot`이 `search_summary`를 실제로 설정한 노드 해시만 수집. 이전 세션에서 로드된 이웃 노드의 `search_summary`가 GraphToLang 컨텍스트에 누출되지 않도록 격리. |
