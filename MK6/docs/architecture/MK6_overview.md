...
Think 루프 (필요 횟수만큼)
  ...
  │           └─ ingest 완료 후, 함께 등장한 노드들 간 co_occurrence 엣지 생성 (노드 추가 없음)
...
| 한글 1자 토큰 | 필터링 없이 노드로 생성 ("이"/"가"/"을"도 의미 단위) |
...
| 검색 ingest 노드 간 엣지 | `_fill_empty_slots` 완료 후, 함께 ingest된 노드들 간 `provenance_source="search"`, `proposed_connect_type="co_occurrence"` 엣지 생성. 새 노드 생성 없음 |
