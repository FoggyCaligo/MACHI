...
| 토큰 중요도 필터링 | LangToGraph에서 문장별 centroid 임베딩 기반 cosine 유사도로 중요도 점수 산출. 상위 `TOKEN_IMPORTANCE_RATIO(20%)` 토큰만 노드로 생성. 임베딩 없는 경우 레이블 길이 폴백. 최소 2개 보장. 파티클·구두점이 자연스럽게 걸러지며 노드/엣지 폭발 방지. |
