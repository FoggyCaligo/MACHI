...
| 토큰 중요도 필터링 | LangToGraph에서 문장별 **near + far 방식** 적용. centroid 근접 `TOKEN_IMPORTANCE_NEAR_RATIO(20%)` (문장 대표 개념) + centroid 원거리 `TOKEN_IMPORTANCE_FAR_RATIO(20%)` (고유명사·도메인 특이 개념)의 합집합을 노드로 생성. 단순 상위 N% 방식에서 탈락하던 특이 도메인 개념(고유명사 등)을 포착. 임베딩 없는 경우 레이블 길이 폴백. 최소 2개 보장. |
| EmptySlot 검색 전략 | `user_input` 원문으로 1회 검색 (없으면 hint 합산 폴백). LangToGraph 재파싱 없음 → ingest + payload 저장 |
| 검색 결과 활용 방식 | `payload["search_summary"]`에 저장 (최대 800자). GraphToLang이 `[검색 컨텍스트]` 섹션으로 LLM에 주입. **known_hashes 노드의 search_summary는 제외** (이전 세션 검색 결과 오염 방지) |
| 검색 ingest 노드 간 엣지 | `_fill_empty_slots` 완료 후 co_occurrence 엣지 생성 (노드 추가 없음). ① ingest↔ingest: 같은 쿼리 내 신규 개념들 간. ② ingest↔ConceptPointer: 신규 개념 ↔ known 개념. 상한 없음 (업스트림 토큰 필터로 노드 수가 이미 제한됨). |
| ThoughtEngine 인터페이스 | `lang_to_graph_fn` 파라미터 제거. 검색 결과를 재번역하지 않으므로 불필요 |
| GraphToLang 핵심/참고 분류 | trust 임계값(0.3) 폐기. `known_hashes` 기반으로 교체. think() 시작 시점에 이미 DB에 존재하던 ConceptPointer → 핵심 키워드. 신규 ingest → 참고 개념 |
