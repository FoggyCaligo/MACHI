...
이것이 **검색과 신규 ingest의 트리거**다.

```
EmptySlot 처리 경로:

EmptySlot 발생 (LangToGraph 반환 후)
  │
  ▼
[검색 시도]
  QuestionSlotPlanner(EmptySlot.concept_hint) → 검색 쿼리
  외부 검색 실행
  LangToGraph(검색결과) → TranslatedGraph
  TranslatedGraph의 ConceptPointer로 EmptySlot 교체 시도
  │
  ├─ [채워짐] → ConceptPointer로 교체
  │
  └─ [여전히 빈 경우] → GraphIngestService → 신규 노드 생성
                          → concept_hint를 초기 레이블(labels[0])로 임시 등록
                          → 생성된 노드 주소 → ConceptPointer로 교체
```

EmptySlot = 새로운 개념 학습의 입구.  
모든 EmptySlot이 채워진 후 Activation으로 진행한다.  
이 처리가 완료된 시점에 그래프는 이미 안정화되어 있고, Think는 이 위에서 단일 패스로 수행된다.
