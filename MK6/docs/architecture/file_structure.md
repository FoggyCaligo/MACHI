...
| `TOKEN_IMPORTANCE_NEAR_RATIO` | `0.20` | centroid 근접 토큰 비율 → near 그룹 (핵심 키워드 후보) |
| `TOKEN_IMPORTANCE_FAR_RATIO` | `0.20` | centroid 원거리 토큰 비율 → far 그룹 (참고 개념 후보) |
| `TOKEN_IMPORTANCE_MIN` | `1` | near 그룹 최소 보장 토큰 수 (극단적으로 짧은 문장 대비) |
...
TranslatedGraph    번역 결과 전체 (nodes, edges, source, near_refs, far_refs)
                   - near_refs: centroid 근접 그룹 (핵심 키워드 후보)
                   - far_refs:  centroid 원거리 그룹 (참고 개념 후보)
                   - 두 필드는 언어 구조 기반, 그래프 상태와 무관
```
...
| `get_edge_by_endpoints` | source→target 방향 active 엣지 조회 (endpoints 기준 강화에 사용) |
...
- `_TOKEN_RE = r"[A-Za-z0-9][A-Za-z0-9_+\-./#]*|[가-힣]{2,}"` — 한글 **2자 이상**만 추출. 영숫자 시작을 요구하여 단독 구두점("." 등) 제외. 1자 한글 토큰("은", "이" 등)은 정규식 수준에서 차단.
- `_KO_PARTICLES` — 한국어 조사·어미 목록 (길이 내림차순). `extract_tokens` 에서 2자 이상 한글 토큰의 접미 조사를 **한 번만 제거(strip)**하여 어간만 남긴다. 제거 후 어간이 2자 미만이면 제거하지 않음(MK5 정책).
...
4. **near/far 분리** (`_split_near_far`): 문장별로 토큰을 두 그룹으로 분류 — 그래프 상태와 무관, 언어 구조만 기준
   - `_importance_scores`: 문장 centroid 임베딩과의 cosine 유사도. 임베딩 없는 토큰은 레이블 길이 기반 폴백.
   - **near** (`TOKEN_IMPORTANCE_NEAR_RATIO=0.20`): centroid 근접 토큰 → `TranslatedGraph.near_refs` → GraphToLang **핵심 키워드**
   - **far** (`TOKEN_IMPORTANCE_FAR_RATIO=0.20`): centroid 원거리 토큰 → `TranslatedGraph.far_refs` → GraphToLang **참고 개념**
   - near/far 겹치면 near 우선. near 최소 `TOKEN_IMPORTANCE_MIN(1)`개 보장.
   - **이 분리가 전체 파이프라인의 유일한 노이즈 제어 포인트**. 하위 컴포넌트(ThoughtEngine, GraphToLang)는 별도 상한을 두지 않는다.
5. near+far 합집합을 원래 sentence 순서로 재구성 → 노드 추가. 인접 통과 쌍 → `neutral` TranslatedEdge (관계 타입 확정은 ThoughtEngine)
...
`GraphDelta`: `added_nodes`, `modified_nodes`, `added_edges`, `removed_edges` — `is_empty()`로 수렴 판단에 사용. `reset_delta()`로 회차마다 초기화.

커밋 추적: `all_added_node_hashes` / `all_added_edge_ids` — 루프 전체 누적, `reset_delta()`로 초기화되지 않음. `ThoughtEngine._commit_new_content`가 이 목록을 사용해 다회 루프에서도 이전 회차 추가분이 누락되지 않도록 한다.
...
TempThoughtGraph 초기 구성:
- `load_from_translated`: ConceptPointer LocalSubgraph 로드 + EmptySlot 등록
- `_add_translated_edges` (1차): CP↔CP TranslatedEdge 즉시 변환 (`edge_weight=confidence`, `provenance="lang_to_graph"`)
- EmptySlot 포함 엣지는 fill 후 2차 호출에서 처리

루프 한 회차:
1. EmptySlot → `user_input` 원문으로 1회 검색 → `_ingest_slot(slot, search_text)` → 노드 생성 + payload 저장  
   → fill 직후 `_add_translated_edges` (2차): EmptySlot이 채워진 TranslatedEdge 추가
...
- search_fn 전체에 `asyncio.wait_for(timeout=SEARCH_TIMEOUT)` 적용 — 초과 시 검색 없이 진행.

co_occurrence 엣지 생성:
- `_fill_empty_slots` 완료 후, 같은 쿼리에서 등장한 ingest 노드들 간 엣지 생성.
- ① **ingest ↔ ingest**: `provenance_source="search"`, `proposed_connect_type="co_occurrence"`, `edge_weight=0.5` (순수 근접 관계)
- ② **ingest ↔ ConceptPointer** (`concept_hashes` 기준 = key_hashes ∪ ref_hashes): 신규 개념 ↔ 기존 개념. `edge_weight=0.6` (의미 연관). 근거 연결에 함께 표시됨.
- 상한 없음 — LangToGraph의 near+far 토큰 필터(각 20%)로 노드 수가 이미 제어되어 있음.

key/ref hashes 수집:
- `think()` 시작 시 `translated.near_refs`의 ConceptPointer → `key_hashes`, `translated.far_refs`의 ConceptPointer → `ref_hashes`.
- Think 루프 종료 후 EmptySlot에서 생성된 노드도 동일 그룹 기준으로 분류 (near_refs EmptySlot → key, far_refs EmptySlot → ref).
- `ConclusionView.key_hashes` / `ref_hashes`로 전달되어 `GraphToLang`의 핵심/참고 분류 기준이 된다.
- 분류 기준은 언어 구조(centroid 거리)이며 그래프 상태(DB 존재 여부)와 무관하다.
...
엣지 강화(reinforcement): `_commit_edge(strong=True)`는 edge_id로 먼저 조회하고, 없으면 `get_edge_by_endpoints`로 같은 src→tgt 관계를 탐색한다. 기존 엣지가 있으면 `edge_weight` 누적 + `support_count` 증가로 강화하고 `update_edge`를 실행. 없으면 신규 `insert_edge`. 이를 통해 반복 등장하는 관계는 weight가 자연스럽게 성장하며 상한 없이 누적된다.
...
| `key_hashes` | `set[str]` | near 그룹 hash 집합 → GraphToLang 핵심 키워드. 언어 구조 기반, 그래프 상태 무관 |
| `ref_hashes` | `set[str]` | far 그룹 hash 집합 → GraphToLang 참고 개념. 언어 구조 기반, 그래프 상태 무관 |
...
| `chat(system, user, model)` | `POST /api/chat` | system/user 메시지 분리 전달. GraphToLang이 사용 |
| `list_models()` | `GET /api/tags` | 텍스트 생성 가능 모델 목록 반환. 두 단계 필터: ① `OLLAMA_EXCLUDED_MODELS` 명시적 제외 (패밀리 메타데이터로 구분 불가한 임베딩 전용 모델) ② Ollama `details.families` 기준 임베딩 전용 패밀리(`nomic-bert`, `bert`, `clip`) 제외 |

`generate`/`chat`은 모델명 미설정 시 `ValueError` 발생.
...
- DuckDuckGo: `asyncio.to_thread` 1개 스레드 내에서 kr-ko + us-en 두 지역 순차 검색. 타임아웃: `DDGS(timeout=8)` + `asyncio.wait_for(12s)`.
- Wikipedia: `/w/api.php` 제목 검색 → `/api/rest_v1/page/summary/{title}` 병렬 fetch
- DDG + Wikipedia 두 경로를 `asyncio.gather`로 동시 실행 후 결과 합산
...
- `graph_to_lang(conclusion)` — ConclusionView → 프롬프트 직렬화 → `chat()` → 언어 출력
  - 핵심 키워드: `key_hashes` 소속 노드 (near 그룹, abstract 제외)
  - 참고 개념: `ref_hashes` 소속 노드 (far 그룹, abstract 제외)
  - 두 분류 모두 언어 구조 기반(centroid 거리), 그래프 상태(DB 존재 여부)와 무관
  ...
  - 검색 컨텍스트: `payload["search_summary"]`가 있는 노드의 요약 포함 (중복 제거, 최대 2개)
