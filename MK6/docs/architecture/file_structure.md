...
- `trust_score < trust_threshold` 노드는 포함하지 않음
...
`ConclusionView`: `nodes`, `edges`, `goal_hash`, `had_empty_slots`, `loop_count`
...
### ollama_client.py — `get_embedding`, `generate`

| 함수 | 엔드포인트 | 설명 |
|---|---|---|
| `get_embedding(text)` | `POST /api/embeddings` | nomic-embed-text 임베딩 벡터 반환 |
| `generate(prompt)` | `POST /api/generate` | 텍스트 생성 (non-streaming) |

`generate`는 `OLLAMA_MODEL_NAME` 미설정 시 `ValueError` 발생.
