...
- `_TOKEN_RE = r"[A-Za-z0-9][A-Za-z0-9_+\-./#]*|[가-힣]+"` — 한글 1자도 토큰. 영숫자 시작을 요구하여 단독 구두점("." 등) 제외.
- `_KO_PARTICLES` — 한국어 조사·의존형태소 불용어 집합. `extract_tokens` 결과에서 단독 조사 토큰을 제거. 대명사(그, 난, 나 등)는 의미 개념으로 취급하여 제외하지 않음.
...
- ② **ingest ↔ ConceptPointer** (known_hashes 기준): 신규 개념 ↔ 기존 개념. `edge_weight=0.6` (의미 연관). 근거 연결에 함께 표시됨.
