...
| opposite | 대비축 활성. 삭제하지 않고 contrast 후보로 남김. |
| conflict | 결론 제거가 아니라 conflict_pressure와 conflict_paths로 기록. |
...
단기 호환을 위해 기존 필드 `nodes`, `edges`, `key_hashes`, `ref_hashes`는 유지할 수 있다.  
...
9. abstract node는 결론 자체가 아니라 conclusion graph 내부의 bridge로 사용한다.

10. GraphToLang은 장기적으로 노드 목록이 아니라 ConclusionGraph를 언어화해야 한다.
```
...
2. GoalView와 TurnGoalView를 TempThoughtGraph에 로드
3. input/goal frontier 기반 ActivationState 계산
4. CandidateConclusionGraph 생성기 구현
5. restatement graph 강등 로직 구현
6. GraphToLang을 selected_graphs 기반으로 전환
```
