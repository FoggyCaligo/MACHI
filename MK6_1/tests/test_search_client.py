...
from MK6_1.tools.search_client import SearchBundle, SearchResult, search_structured
...
class SearchClientTest(unittest.IsolatedAsyncioTestCase):
    async def test_search_structured_returns_empty_bundle_when_all_sources_fail(self) -> None:
        ...
        with (
            patch("MK6_1.tools.search_client._ddg_search", side_effect=fail_ddg),
            patch("MK6_1.tools.search_client._wiki_search", side_effect=fail_wiki),
        ):
    ...
    async def test_search_structured_keeps_successful_sources_when_wiki_fails(self) -> None:
        ...
        with (
            patch("MK6_1.tools.search_client._ddg_search", side_effect=ok_ddg),
            patch("MK6_1.tools.search_client._wiki_search", side_effect=fail_wiki),
        ):
            bundle = await search_structured("그래프")
